...
type TokenAuthProviderTestSuite struct {
...
func (s *TokenAuthProviderTestSuite) TestTokenAuthProviderForApi() {
	...
	gomonolith.TestHTTPResponse(s.T(), s.App, req, func(w *httptest.ResponseRecorder) bool {
		...
		req1, _ := http.NewRequest("GET", "/auth/token/status/", nil)
		req1.Header.Set(
		...
		req1, _ = http.NewRequest("POST", "/auth/token/logout/", nil)
		req1.Header.Set(
			"X-" + core.CurrentConfig.D.GoMonolith.APIToken,
			token.Token,
		)
...
func (s *TokenAuthProviderTestSuite) TestSignupForApi() {
	// hashedPassword, err := utils2.HashPass(password, salt)
	var jsonStr = []byte(`{"username":"test", "confirm_password": "12345678", "password": "12345678", "email": "go-monolithapitest@example.com"}`)
	req, _ := http.NewRequest("POST", "/auth/token/signup/", bytes.NewBuffer(jsonStr))
	req.Header.Set("Content-Type", "application/json")
	gomonolith.TestHTTPResponse(s.T(), s.App, req, func(w *httptest.ResponseRecorder) bool {
		assert.Equal(s.T(), w.Code, 200)
		return w.Code == 200
...
func TestTokenAuthProvider(t *testing.T) {
	gomonolith.RunTests(t, new(TokenAuthProviderTestSuite))
}
